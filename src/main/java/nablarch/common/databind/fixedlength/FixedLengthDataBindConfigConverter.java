        ...
        for (final Annotation annotation : propertyDescriptor.getReadMethod().getAnnotations()) {
            ...
            if (fieldConvert != null) {
                ...
                try {
                    ...
                    throw new IllegalStateException("No constructor is defined for FieldConverter with argument nablarch.common.databind.fixedlength.Field. " +
                            "FieldConverter: " + fieldConvert.value().getName());
                }

                try {
                    fieldConverter = constructor.newInstance(annotation);
                } catch (Exception e) {
                    throw new IllegalStateException("FieldConverter instance creation failed. FieldConverter: " + fieldConvert.value().getName());
                }
            }
        }
